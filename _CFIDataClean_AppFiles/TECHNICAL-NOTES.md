...
A normal cleaning Run proceeds without an AI setup prompt. If **Use AI guidance in export** is checked and AI setup is incomplete, CFI DataClean lists the missing settings and lets the user continue without AI or stop to finish setup.
...
Shared release folders should include the app build number in the folder name, using this format: `CFIDataClean-v1.0.01`. The folder version should match `$script:AppVersion` in `_CFIDataClean_AppFiles\ForestInventoryCleaner.ps1`.
